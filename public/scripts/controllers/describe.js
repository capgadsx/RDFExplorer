...
function DescribeCtrl($scope, pGraph, query, request, settings, log) {
  ...
  function load(uri, sourceObject) {
    ...
    request.execQuery(query.getProperties(uri), {
      callback: data => {
        ...
        properties.forEach(r => {
          ...
          if (cfg.image.includes(obj.uri)) {
            ...
            request.execQuery(query.countValuesType(selected.uri, obj.uri), {
              callback: d => {
  ...
  function loadPropUri(prop) {
    request.execQuery(query.getPropUri(vm.selected.uri, prop), {
      callback: data => {
  ...
  function loadDatatype(prop) {
    request.execQuery(query.getPropDatatype(vm.selected.uri, prop), {
      callback: data => {
  ...
  function loadObject(prop) {
    request.execQuery(query.getPropObject(vm.selected.uri, prop), {
      callback: data => {
